...
public class ContactMenu(ContactController contactController, EmailController emailController)
{
    public void Menu(Contact contact, bool isDeleted)
    {
        while (true)
        {
            Console.Clear();
            if (isDeleted) break;
            AnsiConsole.Write(new Panel(new Markup($"[seagreen1][bold]Chosen contact: {contact.Name}[/][/]").Centered()).Expand());
    ...
    protected void SendEmail(Contact contact)
    {
        ...
        if (message == "0") return;
        emailController.SendEmail(contact, message);
    }
